...
// data = a function returning the content (actually HTML)
	...
	Vue.component('challenge', {
		props: ['layer', 'data'],
		template: `
		<div v-if="tmp[layer].challenges && tmp[layer].challenges[data]!== undefined && tmp[layer].challenges[data].unlocked && !(player.hideChallenges && fullyCompletedChallenges(layer, [data]))" v-bind:class="{hChallenge: true, done: fullyCompletedChallenges(layer, data), canComplete: player[layer].activeChallenge == data&&!fullyCompletedChallenges(layer, data)&&canCompleteChallenge(layer, data)}">
			<br><h3 v-html="tmp[layer].challenges[data].name"></h3><br><br>
			<button v-bind:class="{ longUpg: true, can: true, [layer]: true }" v-bind:style="{'background-color': tmp[layer].color}" v-on:click="startChallenge(layer, data)">{{player[layer].activeChallenge==(data)?(canCompleteChallenge(layer, data)?"Finish":"Exit Early"):(fullyCompletedChallenges(layer, data)?"Completed":"Start")}}</button><br><br>
